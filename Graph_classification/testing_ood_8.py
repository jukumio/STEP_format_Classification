...
class IntegratedAnalyzer:
    ...
    def extract_true_class_from_path(self, file_path):
        ...
        # Strategy 1: Look for '--' delimiter (folder naming pattern)
        full_path = '/'.join(path_parts)
        if '--' in full_path:
            # Find the part after '--' 
            for part in path_parts:
                if '--' in part:
                    class_key = part.split('--')[1]
                    # Direct mapping check
                    if class_key in self.class_to_label:
                        return class_key
                    ...
                    folder_to_class_mapping = {
                        ...
                        'pipes': 'Ironbar_like',
                        'shaft': 'Ironbar_like',
                        'sprocket': 'Power_transmission',
                        'gear': 'Power_transmission',
                        'hinge': 'Hinges',
                    }
                    
                    mapped = folder_to_class_mapping.get(class_key)
                    if mapped and mapped in self.class_to_label:
                        return mapped
        
        # Strategy 2: Look for testing_ood structure but go deeper
        try:
            testing_ood_index = path_parts.index('testing_ood')
            remaining_parts = path_parts[testing_ood_index + 1:]
            
            # Check each remaining part for direct class match
            for part in remaining_parts:
                if part in self.class_to_label:
                    return part
            
            # Check for keyword matching in any remaining part
            folder_to_class_mapping = {
                'screw': 'Screw_like',
                'bolt': 'Screw_like', 
                'pipe': 'Ironbar_like',
                ...
                'bracket': 'Brackets',
                'nut': 'Nuts',
                'fastener': 'Toroidal_fastener',
                'toroidal': 'Toroidal_fastener',
            }
            
            for part in remaining_parts:
                part_lower = part.lower()
                for keyword, mapped_class in folder_to_class_mapping.items():
                    if keyword in part_lower and mapped_class in self.class_to_label:
                        return mapped_class
            
            # If no matches, try the immediate folder after testing_ood
            if remaining_parts:
                main_folder = remaining_parts[0]
                
                # Check if it's directly in class mapping
                if main_folder in self.class_to_label:
                    return main_folder
                
                # Apply keyword matching to main folder
                main_lower = main_folder.lower()
                for keyword, mapped_class in folder_to_class_mapping.items():
                    if keyword in main_lower and mapped_class in self.class_to_label:
                        return mapped_class
                        
        except ValueError:
            pass
        
        # Strategy 3: General keyword search in entire path
        ...
        # Strategy 4: Fallback - use last directory name
        if len(path_parts) >= 2:
            fallback_folder = path_parts[-2]  # Parent directory of file
            
            if fallback_folder in self.class_to_label:
